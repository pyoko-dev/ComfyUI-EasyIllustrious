...
class IllustriousPoses:
    ...
    def INPUT_TYPES(cls) -> Dict:
        ...
        "Add Chain Insert Point": (
                    ...
                    {
            "default": False,
            "tooltip": f"Insert a placeholder token ('{IllustriousPoses.CHAIN_INSERT_TOKEN}') so the next node can inject its content here.",
                    },
                ),
            },
            "required": {
                "Descriptive (BETA)": (
                    ...
                    {
            "default": False,
            "tooltip": "Use natural-language description instead of key tag.",
                    },
                ),
                "Pose Weight": (
                    "FLOAT",
                    {
                        ...
                        "max": 2.0,
                        "step": 0.1,
            "tooltip": "Weight applied to the pose tag, e.g. (tag:weight).",
                    },
                ),
                "Pose Set": (
                    ["-"] + list(IllustriousPoses.PoseTokens.__members__.keys()),
                    {
            "default": "-",
            "tooltip": "Choose a pose tag to insert.",
                    },
                ),
                "Format": (
                    "BOOLEAN",
                    {
            "default": False,
            "tooltip": "Output KEY form (LOW_SQUAT) instead of words.",
                    },
                ),
                "Offset (Beta)": (
                    "INT",
                    {
                        "default": 0,
                        "min": 0,
            "tooltip": "How many words back from the end of prefix to insert (0=end).",
                    },
                ),
            },
        }
    ...
    def construct(self, **kwargs) -> tuple:
        ...
        if pose_set_one_name != "-":
            if not kwargs.get("Descriptive (BETA)"):
                ...
                if kwargs.get(
                    "Format", True
                ):  # 'Format' here probably means 'replace underscores'
                ...
                try:
                    pose_text = IllustriousPoses.PoseTokens[pose_set_one_name].value
                except KeyError:
                    ...
                    if (
                        pose_set_one_name == "custom_raw_pose"
                    ):  # Or some other indicator for raw input
        ...
        offset_for_non_token_insert = kwargs.get(
            "Offset", 0
        )  # This node's specific offset

        if IllustriousPoses.CHAIN_INSERT_TOKEN in prefix_input_str:
            parts = prefix_input_str.split(IllustriousPoses.CHAIN_INSERT_TOKEN, 1)
            ...
            temp_parts = [
                p for p in [prefix_head, this_node_core_content_str] if p and p.strip()
            ]
            working_prompt_str = ", ".join(temp_parts).strip()
            working_prompt_str = re.sub(
                r"\s*,\s*", ", ", working_prompt_str
            )  # Clean commas
            ...
            if prefix_input_str and this_node_core_content_str:
                ...
                prefix_words.insert(
                    insertion_index, this_node_core_content_str
                )  # Insert the pose tag
                ...
                working_prompt_str = re.sub(
                    r"\s*,\s*",
                    ", ",
                    working_prompt_str.replace(
                        f" {this_node_core_content_str}",
                        f", {this_node_core_content_str}",
                    ),
                ).strip()
        ...
        # This node doesn't have complex "modifiers" like earlier variants. Token is added after its content.
        prompt_after_own_token_add = working_prompt_str
        if add_insert_point_for_next_node:
            if (
                prompt_after_own_token_add
            ):  # If there's content, add token after a comma (if style is comma-sep)
                prompt_after_own_token_add = f"{prompt_after_own_token_add}, {IllustriousPoses.CHAIN_INSERT_TOKEN}"
            else:  # If no content yet, token is the first thing
                prompt_after_own_token_add = IllustriousPoses.CHAIN_INSERT_TOKEN
        ...
        temp_parts_for_final_join = [
            prompt_after_own_token_add,
            deferred_suffix_from_parent,
            suffix_chain_input_str,
        ]
        final_prompt_str = ", ".join(
            [p for p in temp_parts_for_final_join if p and p.strip()]
        ).strip()
        ...
        final_prompt_str = final_prompt_str.replace(
            f", {IllustriousPoses.CHAIN_INSERT_TOKEN}",
            f" {IllustriousPoses.CHAIN_INSERT_TOKEN}",
        )
        final_prompt_str = final_prompt_str.replace(
            f"{IllustriousPoses.CHAIN_INSERT_TOKEN},",
        ...
        final_prompt_str = re.sub(
            r"\s{2,}", " ", final_prompt_str
        ).strip()  # Consolidate spaces
